...
I've been writing and testing everything on a {link-fedora} 37 laptop with `make`, `git`, `fedpkg`, and `podman` installed.
...
In this step, we're essentially following the {link-llvm-pgo-documentation} for how to build a PGO instumented clang. In this step we're going to create PGO instrumented LLVM packages and host them for later consumption on a Copr project. The resulting `clang` will generate profile data upon execution and we're trying to collect, bundle, and merge it for optimizing a rebuild of the LLVM toolchain later (<<step7>>). But rest assured, you don't need to run this on your own. The {link-llvm-pgo-instrumented} project is ready for you to consume in the next steps. So you're free to continue with <<step1>>.
...
We also have to add the `lsof` package because of a slightly more advanced topic that we will cover at the end of this section:

[source,dockerfile]
----
include::step2/Containerfile[tag=install_lsof]
----

[NOTE]
====
When we move all this to Copr we must add `lsof` to the list of packages that are available for each build without requiring this as a `BuildRequires:` tag. The `copr edit-chroot` offers this option specifically for these kinds of purposes:
...
We must not wait until we merge the raw PGO profiles. Right now we first create many of them in the `%build` section of an RPM spec file and then we're merging it in the '%install' section. This gap causes a lot of disk space to usage that we can avoid by merging regularly in the background.
...
Then, once build is done, we ask the background job to gracefully shut down by sending it a `TERM` signal and waiting for it to finish.

.step2/myapp/myapp.spec
[source,spec]
----
include::step2/myapp/myapp.spec[tags=wait_for_background_merge]
----

===== Background merge

The background script itself gathers all profiles that have been already written to the `TMPDIR` we've specified link:#TMPDIR[here]:

.step2/myapp/background-merge.sh
[source,spec]
----
include::step2/myapp/background-merge.sh[tags=gather_profiles]
----

It then checks (using `lsof`) if there are profiles still in use. That potentially means, that `clang`, `clang++` or `lld` are writing to those files.

.step2/myapp/background-merge.sh
[source,spec]
----
include::step2/myapp/background-merge.sh[tag=get_profiles_in_use]
----

We then look for overlaps in the file listings before we use them for merging:

.step2/myapp/background-merge.sh
[source,spec]
----
include::step2/myapp/background-merge.sh[tags=get_ready_profiles]
----

[TIP]
====
Suppose you have two files with sorted lines of profiles:

1. `profiles.txt` a list of all profiles currently on disk
2. `profiles_in_use.txt` a list of profiles that are currently open by `clang`, `clang++`, `lld` or alike.

And then you want to find out which profiles are good to be merged in the background, that's where the `comm` script comes in handy.

[source,shell]
----
cat <<EOF > profiles.txt
a.profraw
b.profraw
c.profraw
d.profraw
...
[source,console]
----
$ comm -13 profiles_in_use.txt profiles.txt
a.profraw
b.profraw
e.profraw
f.profraw
...
[source,spec]
----
include::step2/myapp/background-merge.sh[tags=merge]
----
...
We export the `LLVM_PROFILE_FILE` environment variable at the right place in the `%__spec_build_pre` macro to kick of the 

.step4/redhat-rpm-config/macros
[source,spec]
----
include::step4/redhat-rpm-config/macros[tags=__spec_build_pre]
----
