...
[NOTE]
====
By default we optimize for each individual architecture. We think that this is good for now. The cases in which you want to cross-compile on one architecture for another exist but are not considered here (for now). Leave a comment if you would like to see this.
==== 
* Benchmark a PGO-optimized LLVM toolchain with CTMark on http://llvm-compile-time-tracker.com/.
