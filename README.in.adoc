...
In all of these repositries I've essentially done the same changes. At first I've added a link:https://rpm-software-management.github.io/rpm/manual/conditionalbuilds.html[build-conditional] that is off by default:
...
----
<1> Here we modify a particular `fedora-37-x86_64` chroot of the {link-llvm-pgo-instrumented} project to use `--with pgo_instrumented_build` when invoking `mock` to build our packages.
...
This step is similar to step 0 in which we've build the PGO instrumented LLVM. Here we're adding a build requirement for `llvm-pgo-profdata`

.step7/llvm/llvm.spec
[source,spec]
----
include::step0/llvm/llvm.spec[tags=pgo_build_requires_optimized]
----

and use the file `%{_libdir}/%{toolchain}-pgo-profdata/llvm-pgo.profdata` as input to `LLVM_PROFDATA_FILE` for the optimization of the `llvm`, `clang` and `lld` packages.
