...
written to the `TMPDIR` directory that we’ve specified
link:#TMPDIR[here]:
...
[arabic]
. `profiles.txt` a list of all profiles currently on disk
. `profiles_in_use.txt` a list of profiles that are currently open by
`clang`, `clang++`, `lld` or alike.

And then you want to find out which profiles are good to be merged in
the background, that’s where the `comm` script comes in handy.
...
We merge the raw profiles and then delete them in order to free up disk
space.
...
place in the `%__spec_build_pre` macro to kick of the
