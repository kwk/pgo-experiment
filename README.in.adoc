...
[source,console]
----
$ sudo dnf install -y git make fedpkg podman fedora-packager krb5-workstation asciidoctor # <1>
$ gem install pygments.rb <2>
$ git clone --recurse-submodules https://github.com/kwk/pgo-experiment.git # <3>
$ cd pgo-experiment # <4>
$ git fetch --recurse-submodules # <5>
$ git submodule foreach 'git reset --hard origin/pgo-experiment' # <6>
$ kinit <FAS_USER>@FEDORAPROJECT.ORG # <7>
----
...
<2> OPTIONAL: Installing this ruby gem is only for building these docs using `make docs`.
<3> Clone the project including submodules.
<4> Navigate to the project's root directory.
<5> In order to update the submodules we have to first fetch information from their remotes (`origin`).
<6> Then walk over each submodule directory and reset it to the `origin/pgp-experiment` branch.
<7> OPTIONAL: If you want to build the steps that involve the `copr` CLI, you need to have a valid Kerberos ticket. Replace `<FAS_USER>` with the your own Fedora FAS user name.
...
I've set up `pgo-experiment` branches in each of those package git repositories on the Fedora Source:

1. link:https://src.fedoraproject.org/fork/kkleine/rpms/llvm/tree/pgo-experiment[llvm]
2. link:https://src.fedoraproject.org/fork/kkleine/rpms/clang/tree/pgo-experiment[clang]
3. link:https://src.fedoraproject.org/fork/kkleine/rpms/lld/tree/pgo-experiment[lld]

In all of these repositries I've essentially done the same changes. At first I've added link:https://rpm-software-management.github.io/rpm/manual/conditionalbuilds.html[build-conditionals] that are off by default:

.step0/llvm/llvm.spec
[source,spec]
----
include::step0/llvm/llvm.spec[tags=pgo_build_conditions]
----
...
[source,spec]
----
include::step0/llvm/llvm.spec[lines=319..323]
----
...
// TOOD(kwk): Show Results here for an example not done with clang but with the hello world app itself.
...
We've seen how we can gather PGO profile data and feed it into a PGO optimized recompilation of LLVM.
